...
public class MuteCommand {
    public MuteCommand(MessageReceivedEvent event, String[] messageSplit) {
        ...
        for (int i=0; i<targets.size(); i++) {
            event.getGuild().addRoleToMember(targets.get(i), Objects.requireNonNull(event.getGuild().getRoleById("581143000021991424"))).queue();
        ...
        event.getChannel().sendMessage("Muted " + muteReturnString + ".").queue(botMessage -> {
            botMessage.delete().queueAfter(5, TimeUnit.SECONDS);
        });
    }
}
